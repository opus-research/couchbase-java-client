...
public class NumericRangeFacet extends SearchFacet {
    ...
    protected NumericRangeFacet(String name, String field, int limit, String firstRangeName, Double firstMin, Double firstMax) {
        super(name, field, limit);
        this.numericRanges = new HashMap<String, NumericRange>();

        checkRange(firstRangeName, firstMin, firstMax);
        this.numericRanges.put(firstRangeName, new NumericRange(firstMin, firstMax));
    }

    public void checkRange(String name, Double min, Double max) {
        if (name == null) {
