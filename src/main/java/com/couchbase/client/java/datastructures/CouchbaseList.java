...
public class CouchbaseList<E> {
    ...
    public Observable<Boolean> remove(final E element, final MutationOptionBuilder mutationOptionBuilder) {
        return Observable.defer(new Func0<Observable<Boolean>>() {
            @Override
            public Observable<Boolean> call() {
                return subdocRemove(element, mutationOptionBuilder)
                        .map(new Func1<DocumentFragment<Mutation>, Boolean>() {
                            @Override
                            public Boolean call(DocumentFragment<Mutation> mutationDocumentFragment) {
                                ResponseStatus status = mutationDocumentFragment.status(0);
                                if (mutationDocumentFragment.status(0) == ResponseStatus.SUCCESS) {
                                    return true;
                                } else {
                                    if (status == ResponseStatus.SUBDOC_PATH_NOT_FOUND) {
                                        throw new IndexOutOfBoundsException();
    ...
    private Observable<DocumentFragment<Mutation>> subdocRemove(final E element,
                                                                final MutationOptionBuilder mutationOptionBuilder) {
        return bucket.get(docId, JsonArrayDocument.class)
                .flatMap(new Func1<JsonArrayDocument, Observable<DocumentFragment<Mutation>>>() {
                    @Override
                    public Observable<DocumentFragment<Mutation>> call(JsonArrayDocument jsonArrayDocument) {
                        if (jsonArrayDocument.cas() != mutationOptionBuilder.cas) {
