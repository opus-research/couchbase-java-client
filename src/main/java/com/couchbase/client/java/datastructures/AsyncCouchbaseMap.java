 ...
 * AsyncCouchbaseMap is a map collection like interface supporting CouchbaseMap operations that can be executed
 * asynchronously against a Couchbase Server bucket, here key is a string and value can be a Json data type.
 *
...
public class AsyncCouchbaseMap<V> {
     ...
     */
    public AsyncCouchbaseMap(CouchbaseAsyncBucket bucket, String docId) {
        this.bucket = bucket;
        this.docId = docId;
    }
    ...
    private Observable<DocumentFragment<Mutation>> subdocAdd(final String key,
        ...
        return bucket.mutateIn(docId).upsert(key, value, false)
                .withCas(cas)
