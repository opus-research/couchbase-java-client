...
public class BooleanQueryTest {
    ...
    public void shouldExportBooleanQueryWithInnerBoosts() {
        ...
        JsonObject expectedMustNot = JsonObject.create().put("disjuncts", JsonArray.from(expectedInner)).put("min", 1);
        JsonObject expectedShould = JsonObject.create().put("disjuncts", JsonArray.from(expectedInner)).put("min", 1);
    ...
    @Test
    public void shouldExportInnerWithoutSearchParams() {
        PrefixQuery inner = SearchQuery.prefix("someterm", SearchParams.build().explain()).boost(2);
        BooleanQuery query = SearchQuery.booleans().must(inner).mustNot(inner).should(inner);
    ...
    public void shouldUseMinForShould() {
        PrefixQuery inner = SearchQuery.prefix("someterm", SearchParams.build().explain()).boost(2);
        BooleanQuery query = SearchQuery.booleans().must(inner).mustNot(inner).should(inner, inner).minShould(2);

        JsonObject expectedInner = JsonObject.create().put("prefix", "someterm").put("boost", 2.0);
        //default min from a disjunction query
        JsonObject expectedMustNot = JsonObject.create().put("disjuncts", JsonArray.from(expectedInner)).put("min", 1);
    ...
    public void shouldFailOnShouldSectionTooSmall() {
        PrefixQuery inner = SearchQuery.prefix("someterm", SearchParams.build().explain()).boost(2);
        BooleanQuery query = SearchQuery.booleans().should(inner).minShould(2);
    ...
    public void shouldExportBooleanQueryWithAllOptions() {
        ...
        BooleanQuery query = SearchQuery.booleans(SearchParams.build().explain())
            .boost(1.5)
            .must(innerA)
            .mustNot(innerB)
