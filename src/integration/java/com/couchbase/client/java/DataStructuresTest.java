...
public class DataStructuresTest {
    ...
    public void testMapRequestTooBigException() {
        ...
        boolean result = ctx.bucket().async().mapAdd("dsmapFull", "foo", str, MutationOptionBuilder.build().withDurability(PersistTo.MASTER)).toBlocking().single();
    }
    ...
    public void testMapExpiry() {
        ctx.bucket().mapAdd("dsmapShortLived", "1", "1", MutationOptionBuilder.build().withExpiry(1));
        try {
            Thread.sleep(2000);
    ...
    public void testList() {
        ...
        ctx.bucket().async().listSet("dslist", 3, 10).toBlocking().single();
        int intVal = ctx.bucket().async().listGet("dslist", 3, Integer.class).toBlocking().single();
    ...
    public void testListExpiry() {
        ctx.bucket().listPush("dslistShortLived", "1", MutationOptionBuilder.build().withExpiry(1));
        try {
            Thread.sleep(2000);
        } catch (InterruptedException ex) {
        }
    ...
    public void testQueue() {
        ...
        assertNotNull(first);
        boolean result = ctx.bucket().async().queueAdd("dsqueue", "val1").toBlocking().single();
        assert (result == true);
        result = ctx.bucket().async().queueAdd("dsqueue", "val2").toBlocking().single();
        assert (result == true);
        String val = ctx.bucket().async().queueRemove("dsqueue", String.class).toBlocking().single();
        assertEquals(val, "val1");
        val = ctx.bucket().async().queueRemove("dsqueue", String.class).toBlocking().single();
    ...
    public void testQueueExpiry() {
        ctx.bucket().queueAdd("dsqueueShortLived", "1", MutationOptionBuilder.build().withExpiry(1));
        try {
            Thread.sleep(2000);
        } catch (InterruptedException ex) {
        }
    ...
    public void testSetFullException() {
        char[] data = new char[5000000];
        String str = new String(data);
        boolean result = ctx.bucket().async().setAdd("dssetFull", str, MutationOptionBuilder.build().withDurability(PersistTo.MASTER)).toBlocking().single();
        assertEquals(result, true);
    }
    ...
    public void testSetExpiry() {
        ctx.bucket().setAdd("dssetShortLived", "1", MutationOptionBuilder.build().withExpiry(1));
        try {
            Thread.sleep(2000);
        } catch (InterruptedException ex) {
        }
